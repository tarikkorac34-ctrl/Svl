...
// Map team keys to possible Discord emoji names for auto-matching
const TEAMS = {
...
// ============================================================
// CANVAS TABLE RENDERER (ADJUSTED FIT COORDS)
// ============================================================
async function generateStandingsImage(title, rows) {
  ...
  // Dark Inner Box Container
  const cardX = 60;
  const cardWidth = canvasWidth - 120; // 880px wide container
  ctx.fillStyle = 'rgba(15, 23, 42, 0.90)';
  ctx.roundRect(cardX, 40, cardWidth, canvasHeight - 80, 16);
  ctx.fill();
  ...
  // Header Coordinates (P starts at 430, columns tightly spaced)
  ctx.font = fontHeader;
  ctx.fillStyle = '#38BDF8';

  ctx.textAlign = 'left';
  ctx.fillText('POS', 90, startY);
  ctx.fillText('TEAM', 170, startY);

  ctx.textAlign = 'right';
  ctx.fillText('P',   430, startY);
  ctx.fillText('W',   480, startY);
  ctx.fillText('D',   530, startY);
  ctx.fillText('L',   580, startY);
  ctx.fillText('GD',  640, startY);
  ctx.fillText('PTS', 710, startY);
  ...
  ctx.lineTo(canvasWidth - 80, startY + 20);
  ctx.stroke();
  ...
  rows.forEach((row, index) => {
    ...
    if (index % 2 === 0) {
      ...
      ctx.fillRect(80, y - 22, canvasWidth - 160, rowHeight - 8);
    }
    ...
    ctx.fillText(`${row.teamName}`, 170, y);

    // Numeric Columns (Matching exact header positions)
    ctx.textAlign = 'right';
    ctx.fillText(`${row.GP}`, 430, y); // P
    ctx.fillText(`${row.W}`,  480, y); // W
    ctx.fillText(`${row.D}`,  530, y); // D
    ctx.fillText(`${row.L}`,  580, y); // L
    ...
    ctx.fillText(`${row.GD > 0 ? '+' + row.GD : row.GD}`, 640, y);

    // PTS
    ctx.fillStyle = '#FACC15';
    ctx.font = fontMain;
    ctx.fillText(`${row.PTS}`, 710, y);
  });
...
async function handleButton(interaction) {
  ...
  // Direct Message Notification sent to the requester
  try {
...
async function cmdPlayer(i) {
  ...
  // Match player by ID or Username
