...
// Initialize Roblox Authentication with IP-validation bypass
async function initRoblox() {
  if (ROBLOX_COOKIE) {
    try {
      await noblox.setCookie({ cookie: ROBLOX_COOKIE, validate: false });
      console.log(`✅ Set Roblox authentication cookie.`);
    } catch (err) {
      console.error('❌ Roblox Authentication Error:', err.message);
    }
  } else {
...
async function cmdRankRequest(interaction) {
  ...
  // Sends request directly to #rr-logs channel
...
async function handleRankAccept(interaction) {
  ...
  // Update original request embed in #rr-logs
