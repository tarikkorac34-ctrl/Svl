...
// ============================================================
// CANVAS TABLE RENDERER (SHIFTED LEFT TO KEEP PTS INSIDE CARD)
// ============================================================
async function generateStandingsImage(title, rows) {
  ...
  // Dark Inner Card (spans X = 60 to X = 940)
  const cardX = 60;
  ...
  // Header Coordinates (P starts at 480, PTS ends cleanly at 820 inside the card)
  ctx.font = fontHeader;
  ctx.fillStyle = '#38BDF8';

  ctx.textAlign = 'left';
  ctx.fillText('POS', 90, startY);
  ctx.fillText('TEAM', 160, startY);

  ctx.textAlign = 'right';
  ctx.fillText('P',   480, startY);
  ctx.fillText('W',   540, startY);
  ctx.fillText('D',   600, startY);
  ctx.fillText('L',   660, startY);
  ctx.fillText('GD',  740, startY);
  ctx.fillText('PTS', 820, startY);
  ...
  rows.forEach((row, index) => {
    ...
    ctx.fillText(`${row.teamName}`, 160, y);

    // Numeric Columns
    ctx.textAlign = 'right';
    ctx.fillText(`${row.GP}`, 480, y); // P
    ctx.fillText(`${row.W}`,  540, y); // W
    ctx.fillText(`${row.D}`,  600, y); // D
    ctx.fillText(`${row.L}`,  660, y); // L
    ...
    ctx.fillText(`${row.GD > 0 ? '+' + row.GD : row.GD}`, 740, y);

    // PTS
    ctx.fillStyle = '#FACC15';
    ctx.font = fontMain;
    ctx.fillText(`${row.PTS}`, 820, y);
  });
