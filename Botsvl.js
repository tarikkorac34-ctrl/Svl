...
client.once('ready', async () => {
  ...
  try {
    // 1. WIPE OLD GLOBAL COMMANDS (Clears outdated cached parameters in Discord)
    await rest.put(Routes.applicationCommands(client.user.id), { body: [] });
    console.log('🧹 Cleared old global commands.');

    // 2. RE-REGISTER UPDATED GUILD COMMANDS
    for (const guild of client.guilds.cache.values()) {
      await rest.put(Routes.applicationGuildCommands(client.user.id, guild.id), { body: commandDefs });
      console.log(`✅ Registered updated commands in: ${guild.name}`);
    }
  } catch (err) {
