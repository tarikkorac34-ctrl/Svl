...
client.once('ready', async () => {
  ...
  // Also register per-guild for instant availability across all existing servers
  try {
    ...
    console.error('❌ Guild command deployment error:', err);
  }
...
client.on('guildCreate', async (guild) => {
  ...
  try {
    await rest.put(Routes.applicationGuildCommands(client.user.id, guild.id), { body: commandDefs });
    console.log(`✅ Registered commands in newly joined guild: ${guild.name}`);
  } catch (err) {
...
async function cmdResult(interaction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0x00FF57)
    .setDescription(`**${TEAMS[game.homeTeam]?.fullName || game.homeTeam}** ${homeScore} - ${awayScore} **${TEAMS[game.awayTeam]?.fullName || game.awayTeam}**`)
...
async function handleGameAutocomplete(interaction) {
  ...
  const choices = games.map(g => ({
    name: `${g.division} | ${TEAMS[g.homeTeam]?.name || g.homeTeam} vs ${TEAMS[g.awayTeam]?.name || g.awayTeam} (${g.datetimeSwedish})`,
    value: g._id
  }));
