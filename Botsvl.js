...
const MONGO_URI = process.env.MONGODB_URI || 'mongodb+srv://pazaracbfl_db_user:Novipazar78@pazaracbfl.qapkmuh.mongodb.net/svl_database?retryWrites=true&w=majority';
const PORT = process.env.PORT || 3000;
const ROBLOX_API_KEY = process.env.ROBLOX_API_KEY;
const ROBLOX_GROUP_ID = process.env.ROBLOX_GROUP_ID || '529596037';
...
// ============================================================
// ROBLOX & DISCORD RANK CONFIGURATION (USING MAPPED ROLE IDs)
// ============================================================
const RANK_CONFIG = {
  STAFF:       { name: 'Staff',       robloxRoleId: '562002064', discordRole: 'Staff' },
  REFEREE:     { name: 'Referee',     robloxRoleId: '637405083', discordRole: 'Referee Team' },
  TEAM_OWNER:  { name: 'Team owner',  robloxRoleId: '661127004', discordRole: 'Team Owner' },
  PLAYER:      { name: 'Player',      robloxRoleId: '636189032', discordRole: null },
  FAN_LEADER:  { name: 'Fan leader',  robloxRoleId: '559774113', discordRole: null }
};
...
// ============================================================
let mongoUriClean = MONGO_URI.trim();
if (mongoUriClean.startsWith('Mongodb+srv://')) {
  mongoUriClean = mongoUriClean.replace('Mongodb+srv://', 'mongodb+srv://');
}

const mongoClient = new MongoClient(mongoUriClean);
...
async function storePending(type, requesterId, targetId, targetUsername, teamKey, robloxUsername, proofUrl = null) {
  const id = Date.now().toString(36) + Math.random().toString(36).substr(2, 5);
  await pendingCol.insertOne({
    _id: id,
    type, requesterId, targetId, targetUsername, teamKey, robloxUsername, proofUrl,
    createdAt: new Date()
  });
  return id;
}
...
const commandDefs = [
  ...
  new SlashCommandBuilder()
    ...
    .addAttachmentOption(o => o.setName('proof').setDescription('Upload proof image of the signing').setRequired(true)),
...
// ============================================================
// RANK REQUEST SYSTEM (ROBLOX OPENCLOUD ROLE ID VERSION)
// ============================================================
  ...
  if (ROBLOX_GROUP_ID && ROBLOX_API_KEY) {
    try {
      ...
      const response = await fetch(
        `https://apis.roblox.com/cloud/v2/groups/${ROBLOX_GROUP_ID}/memberships/${robloxId}?updateMask=role`,
        {
          method: 'PATCH',
          headers: {
          ...
          body: JSON.stringify({
            role: `groups/${ROBLOX_GROUP_ID}/roles/${rankInfo.robloxRoleId}`
          })
        }
      );

      if (response.status === 404) {
        throw new Error(`User ID ${robloxId} membership or role ID ${rankInfo.robloxRoleId} invalid.`);
      }
      ...
      robloxStatus = `Ranked to ${rankInfo.name} (Role ID: ${rankInfo.robloxRoleId})`;
    } catch (err) {
...
// ============================================================
// SIGN & RELEASE REQUESTS (WITH PROOF ATTACHMENT SUPPORT)
// ============================================================
async function cmdRequestSign(interaction) {
  ...
  // Verify attachment is a valid image file
  if (proofAttachment && !proofAttachment.contentType?.startsWith('image/')) {
    return interaction.editReply({
      content: '❌ Please upload a valid image file (PNG, JPG, WEBP) for proof.'
    });
  }
  ...
  await interaction.editReply({ content: '✅ Sign request submitted successfully with attached image proof!' });
}
