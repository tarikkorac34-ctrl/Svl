...
async function handleButton(interaction) {
  ...
  // Send Direct Message notification to requester
  try {
    ...
    if (requester) {
      ...
      await requester.send(
        `📬 Your request for **${typeText}** <@${req.targetId}> (${req.robloxUsername}) for **${teamName}** has been **${actionText}**.`
      );
    }
  } catch (err) {
    console.error(`Could not DM user ${req.requesterId}:`, err.message);
  }
