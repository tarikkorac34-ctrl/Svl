...
function isStaffOrAdmin(member) {
  ...
  return member.roles.cache.some(role => 
    ['REGISTRATION STAFF', 'STAFF'].includes(role.name.toUpperCase())
  );
}
...
function isRefereeManagerOrStaff(member) {
  ...
  if (member.permissions.has(PermissionFlagsBits.Administrator)) return true;
  return member.roles.cache.some(role => 
    ['REGISTRATION STAFF', 'STAFF', 'TEAM MANAGER'].includes(role.name.toUpperCase())
  );
}

// Permission check for /result (Admins, Staff, or Referee roles)
...
// MVP VOTE HELPERS
// ============================================================
function buildMvpEmbedAndComponents(voteDoc, guild) {
  const homeTeam = TEAMS[voteDoc.homeKey];
  const awayTeam = TEAMS[voteDoc.awayKey];
  const homeEmoji = teamEmoji(guild, voteDoc.homeKey);
  const awayEmoji = teamEmoji(guild, voteDoc.awayKey);

  const totalVotesP1 = voteDoc.votesPlayer1 ? voteDoc.votesPlayer1.length : 0;
  const totalVotesP2 = voteDoc.votesPlayer2 ? voteDoc.votesPlayer2.length : 0;

  const embed = new EmbedBuilder()
    .setTitle('🏆 MVP VOTE')
    .setColor(0xFACC15)
    .setDescription(
      `Vote for the MVP of the match:\n**${homeEmoji} ${homeTeam ? homeTeam.fullName : voteDoc.homeKey} vs ${awayEmoji} ${awayTeam ? awayTeam.fullName : voteDoc.awayKey}**`
    )
    .addFields(
      { 
        name: `Option 1: ${voteDoc.player1Username}`, 
        value: `**${totalVotesP1}** vote(s)`, 
        inline: true 
      },
      { 
        name: `Option 2: ${voteDoc.player2Username}`, 
        value: `**${totalVotesP2}** vote(s)`, 
        inline: true 
      }
    )
    .setFooter({ text: `Vote ID: ${voteDoc._id}` })
    .setTimestamp();

  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`mvp_vote:p1:${voteDoc._id}`)
      .setLabel(`Vote for ${voteDoc.player1Username}`)
      .setStyle(ButtonStyle.Primary),
    new ButtonBuilder()
      .setCustomId(`mvp_vote:p2:${voteDoc._id}`)
      .setLabel(`Vote for ${voteDoc.player2Username}`)
      .setStyle(ButtonStyle.Primary),
    new ButtonBuilder()
      .setCustomId(`mvp_vote:end:${voteDoc._id}`)
      .setLabel('End Vote (Staff)')
...
async function updateFtScoresMvp(guild, gameId, winnerUserId) {
  const resultDoc = await resultsCol.findOne({ gameId });
  if (!resultDoc || !resultDoc.scoresMsgId) return;

  const ftChannel = findChannel(guild, 'ft-scores');
  if (!ftChannel) return;

  try {
    const scoresMsg = await ftChannel.messages.fetch(resultDoc.scoresMsgId);
    if (scoresMsg) {
      const homeEmoji = teamEmoji(guild, resultDoc.homeTeam);
      const awayEmoji = teamEmoji(guild, resultDoc.awayTeam);
      const homeName = TEAMS[resultDoc.homeTeam]?.name || resultDoc.homeTeam;
      const awayName = TEAMS[resultDoc.awayTeam]?.name || resultDoc.awayTeam;
      const divTitle = getDivisionTitle(resultDoc.division);

      const updatedText = 
        `**${divTitle} RESULTS**\n` +
        `Game: ${homeEmoji} **${homeName}** vs **${awayName}** ${awayEmoji}\n` +
        `Result: **${resultDoc.scoreText}**\n` +
        `MVP: <@${winnerUserId}>`;

      await scoresMsg.edit(updatedText);
    }
  } catch (err) {
    console.error('Failed to update #ft-scores message:', err.message);
  }
}
...
const commandDefs = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('game').setDescription('Select finished match').setRequired(true).setAutocomplete(true))
    .addUserOption(o => o.setName('player1').setDescription('First MVP Nominee').setRequired(true))
    .addUserOption(o => o.setName('player2').setDescription('Second MVP Nominee').setRequired(true)),
].map(c => c.toJSON());
...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId.startsWith('mvp_vote:')) {
        return await handleMvpVoteButton(interaction);
      }
...
async function cmdMvpVote(interaction) {
  ...
  const voteId = Date.now().toString(36) + Math.random().toString(36).substr(2, 4);

  const voteDoc = {
    _id: voteId,
    gameId,
    guildId: guild.id,
    homeKey: game.homeTeam,
    awayKey: game.awayTeam,
  ...
  await mvpVotesCol.insertOne(voteDoc);

  const { embed, row } = buildMvpEmbedAndComponents(voteDoc, guild);

  await mvpChannel.send({ embeds: [embed], components: [row] });
  await interaction.editReply({ content: '✅ MVP vote session successfully created in `#mvp`!' });
}

async function handleMvpVoteButton(interaction) {
  const [, targetChoice, voteId] = interaction.customId.split(':');
  const userId = interaction.user.id;

  const voteDoc = await mvpVotesCol.findOne({ _id: voteId });
  if (!voteDoc) {
    return interaction.reply({ content: '⚠️ This MVP voting session no longer exists.', ephemeral: true });
  }

  // Handle Staff force-ending the vote
  if (targetChoice === 'end') {
    if (!isStaffOrAdmin(interaction.member)) {
      return interaction.reply({ content: '❌ Only Staff/Admins can end the vote.', ephemeral: true });
    }

    const votesP1 = voteDoc.votesPlayer1 ? voteDoc.votesPlayer1.length : 0;
    const votesP2 = voteDoc.votesPlayer2 ? voteDoc.votesPlayer2.length : 0;

    let winnerId = null;
    let winnerName = 'Tie / None';

    if (votesP1 > votesP2) {
      winnerId = voteDoc.player1Id;
      winnerName = voteDoc.player1Username;
    } else if (votesP2 > votesP1) {
      winnerId = voteDoc.player2Id;
      winnerName = voteDoc.player2Username;
    }

    if (winnerId) {
      await updateFtScoresMvp(interaction.guild, voteDoc.gameId, winnerId);
    }

    await interaction.update({
      content: `🔒 **MVP Vote Closed!** Winner: <@${winnerId}> (${winnerName})`,
      embeds: [],
      components: []
    });
    return;
  ...
  // Handle standard user voting
  let votesP1 = voteDoc.votesPlayer1 || [];
  let votesP2 = voteDoc.votesPlayer2 || [];

  votesP1 = votesP1.filter(id => id !== userId);
  votesP2 = votesP2.filter(id => id !== userId);

  if (targetChoice === 'p1') votesP1.push(userId);
  if (targetChoice === 'p2') votesP2.push(userId);

  await mvpVotesCol.updateOne(
    { _id: voteId },
    { $set: { votesPlayer1: votesP1, votesPlayer2: votesP2 } }
  );

  const updatedDoc = { ...voteDoc, votesPlayer1: votesP1, votesPlayer2: votesP2 };
  const { embed, row } = buildMvpEmbedAndComponents(updatedDoc, interaction.guild);

  await interaction.update({ embeds: [embed], components: [row] });
  await interaction.followUp({ content: '✅ Your MVP vote has been recorded!', ephemeral: true });
}
...
// ============================================================
// RESULT COMMAND (Restricted to Admins, Staff, & Referees)
// ============================================================
