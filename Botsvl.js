...
async function generateStandingsImage(title, rows) {
  ...
  // Dark Card Background Box
  const cardWidth = canvasWidth - 80;
  ctx.fillStyle = 'rgba(15, 23, 42, 0.90)';
  ctx.roundRect(40, 40, cardWidth, canvasHeight - 80, 16);
  ctx.fill();
  ...
  // Table Headers (Positioned inside card bounds)
  ctx.font = fontHeader;
  ...
  ctx.fillText('GD', 730, startY);
  ctx.fillText('PTS', 810, startY);
  ...
  rows.forEach((row, index) => {
    ...
    // Stats
    ctx.textAlign = 'right';
    ctx.fillText(`${row.GP}`, 480, y);
    ctx.fillText(`${row.W}`, 540, y);
    ctx.fillText(`${row.D}`, 600, y);
    ctx.fillText(`${row.L}`, 660, y);
    ...
    ctx.fillText(`${row.GD > 0 ? '+' + row.GD : row.GD}`, 730, y);

    // Points
    ctx.fillStyle = '#FACC15';
    ctx.font = fontMain;
    ctx.fillText(`${row.PTS}`, 810, y);
  });
