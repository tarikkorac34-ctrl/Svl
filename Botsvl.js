...
function getPending(id) { return loadPending()[id] || null; }
...
async function cmdGame(interaction) {
  ...
  // ==========================================================
  // Create Discord scheduled event (EXTERNAL, ends 2h after kickoff)
  // EXTERNAL event rules enforced by Discord:
  //   • scheduledEndTime is REQUIRED
  //   • entityMetadata.location is REQUIRED, 1–100 characters
  //   • no channel may be passed
  //   • name ≤ 100 chars, description ≤ 1000 chars
  //   • scheduledStartTime must be in the future
  // ==========================================================
  let scheduledEvent = null;
  try {
    const eventName = `${competition}: ${homeTeam.name} vs ${awayTeam.name}`.slice(0, 100);

    // location must be a non-empty string, max 100 chars (this was the crash cause)
    const location = (venue && venue.trim() ? venue.trim() : 'Online').slice(0, 100);
    ...
    // 60s safety buffer so a "start now" event is never rejected as being in the past
    const startTime = new Date(Math.max(utcDate.getTime(), Date.now() + 60_000));
    ...
    scheduledEvent = await guild.scheduledEvents.create({
      ...
      privacyLevel: GuildScheduledEventPrivacyLevel?.GuildOnly ?? ScheduledEventPrivacyLevel?.GuildOnly ?? 2,
      entityType:   GuildScheduledEventEntityType?.External ?? ScheduledEventEntityType?.External ?? 3,
      entityMetadata: { location },
      description: eventDescription,
    });
    console.log(`📅 Scheduled event created: ${homeTeam.name} vs ${awayTeam.name}`);
  } catch (err) {
    // Surface the EXACT field Discord rejected instead of a vague message
    const details = err?.rawError?.errors
      ? JSON.stringify(err.rawError.errors, null, 2)
      : (err?.message ?? String(err));
    console.error('Failed to create scheduled event:', err?.message ?? err);
  ...
  await interaction.editReply({
    content:
      ...
      (scheduledEvent ? `📅 Server event created\n` : `⚠️ Server event could NOT be created (check console logs)\n`) +
