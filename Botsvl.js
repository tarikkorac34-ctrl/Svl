...
// ============================================================
// CANVAS TABLE RENDERER (PERFECT FIT COORDS)
// ============================================================
async function generateStandingsImage(title, rows) {
  ...
  const cardX = 40;
  const cardWidth = canvasWidth - 80; // 920px total card width
  ...
  // Table Headers
  // P starts right at the middle mark (520px) with tight 60px column spacing
  ctx.font = fontHeader;
  ...
  ctx.textAlign = 'right';
  ctx.fillText('P',   520, startY);
  ctx.fillText('W',   580, startY);
  ctx.fillText('D',   640, startY);
  ctx.fillText('L',   700, startY);
  ctx.fillText('GD',  770, startY);
  ctx.fillText('PTS', 850, startY);
  ...
  rows.forEach((row, index) => {
    ...
    // Stats Columns matching the new header coordinates
    ctx.textAlign = 'right';
    ctx.fillText(`${row.GP}`, 520, y); // P
    ctx.fillText(`${row.W}`,  580, y); // W
    ctx.fillText(`${row.D}`,  640, y); // D
    ctx.fillText(`${row.L}`,  700, y); // L
    ...
    ctx.fillText(`${row.GD > 0 ? '+' + row.GD : row.GD}`, 770, y);

    // PTS (Points)
    ctx.fillStyle = '#FACC15';
    ctx.font = fontMain;
    ctx.fillText(`${row.PTS}`, 850, y);
  });
