...
async function generateStandingsImage(title, rows) {
  ...
  const canvasWidth = bgImage ? bgImage.width : 1000;
  const canvasHeight = bgImage ? bgImage.height : 700;
  ...
  // Dark overlay card for readable text
  ctx.fillStyle = 'rgba(15, 23, 42, 0.85)';
  ctx.roundRect(50, 40, canvasWidth - 100, canvasHeight - 80, 20);
  ctx.fill();
  ...
  ctx.fillText(title.toUpperCase(), canvasWidth / 2, 100);
  ...
  ctx.fillText('POS', 90, startY);
  ctx.fillText('TEAM', 180, startY);
  
  ctx.textAlign = 'right';
  ctx.fillText('GP', 560, startY);
  ctx.fillText('W', 630, startY);
  ctx.fillText('D', 700, startY);
  ctx.fillText('L', 770, startY);
  ctx.fillText('GD', 850, startY);
  ctx.fillText('PTS', 920, startY);
  ...
  ctx.lineTo(canvasWidth - 80, startY + 15);
  ctx.stroke();
  ...
  rows.forEach((row, i) => {
    const y = startY + 65 + (i * rowHeight);

    // Alternating row background
    if (i % 2 === 0) {
      ctx.fillStyle = 'rgba(255, 255, 255, 0.05)';
      ctx.fillRect(80, y - 32, canvasWidth - 160, rowHeight - 6);
    }

    ctx.fillStyle = '#F8FAFC';
    ctx.textAlign = 'left';
    ctx.fillText(`${i + 1}`, 100, y);
    ctx.fillText(`${row.teamName}`, 180, y);

    ctx.textAlign = 'right';
    ctx.fillText(`${row.GP}`, 560, y);
    ctx.fillText(`${row.W}`, 630, y);
    ctx.fillText(`${row.D}`, 700, y);
    ctx.fillText(`${row.L}`, 770, y);
    
    ctx.fillStyle = row.GD > 0 ? '#4ADE80' : row.GD < 0 ? '#F87171' : '#F8FAFC';
    ctx.fillText(`${row.GD > 0 ? '+' + row.GD : row.GD}`, 850, y);

    ctx.fillStyle = '#FACC15'; // Gold for points
    ctx.font = 'bold 22px sans-serif';
    ctx.fillText(`${row.PTS}`, 920, y);
    ctx.font = '22px sans-serif';
  });
...
const commandDefs = [
  ...
  new SlashCommandBuilder()
    .setName('standings')
    .setDescription('View the current league table/standings (Staff/Admin Only)')
...
async function cmdStandings(interaction) {
  if (!isStaffOrAdmin(interaction.member)) {
    return interaction.reply({ content: '❌ You need to be an **Administrator** or have the **REGISTRATION STAFF** role to use this command!', ephemeral: true });
  }
  ...
  allowedTeams.forEach(t => {
    table[t] = { teamKey: t, teamName: TEAMS[t]?.fullName || t, GP: 0, W: 0, D: 0, L: 0, GF: 0, GA: 0, GD: 0, PTS: 0 };
  });
  ...
  const sortedRows = Object.values(table).sort((a, b) => b.PTS - a.PTS || b.GD - a.GD || b.GF - a.GF);
...
// --- /requestsign, /requestrelease, /player ---
async function cmdRequestSign(i) {
  await i.deferReply({ ephemeral: true });
  const id = await storePending('sign', i.user.id, i.options.getUser('discord_username').id, i.options.getUser('discord_username').username, i.options.getString('team_name'), i.options.getString('roblox_username'));
  await i.editReply(`✅ Sign request created (${id})`);
}

async function cmdRequestRelease(i) {
  await i.deferReply({ ephemeral: true });
  const id = await storePending('release', i.user.id, i.options.getUser('discord_username').id, i.options.getUser('discord_username').username, i.options.getString('team_name'), i.options.getString('roblox_username'));
  await i.editReply(`✅ Release request created (${id})`);
}
