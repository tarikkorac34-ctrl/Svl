...
// Requires: discord.js ^14, mongodb ^6
// ============================================================
...
async function connectDB() {
  try {
    ...
    // TTL Index for automatic pending request auto-expiry (48 Hours)
...
async function cmdStandings(interaction) {
  ...
  // Initialize Standings Table Map
  const table = {};
  allowedTeams.forEach(t => {
    table[t] = { team: t, GP: 0, W: 0, D: 0, L: 0, GF: 0, GA: 0, GD: 0, PTS: 0 };
  });
  ...
  sorted.forEach((row, i) => {
    ...
    const name = row.team.padEnd(6, ' ');
    const gp = row.GP.toString().padStart(3, ' ');
    const w = row.W.toString().padStart(2, ' ');
    const d = row.D.toString().padStart(2, ' ');
    const l = row.L.toString().padStart(2, ' ');
    const gd = (row.GD >= 0 ? `+${row.GD}` : row.GD).toString().padStart(4, ' ');
    const pts = row.PTS.toString().padStart(4, ' ');
    textTable += `${pos}   ${name}  ${gp} ${w} ${d} ${l} ${gd} ${pts}\n`;
  });
  textTable += '```';

  const embed = new EmbedBuilder()
    .setTitle(`📊 ${divKey === 'DIV1' ? 'Division 1' : 'Division 2'} League Standings`)
    .setColor(0x005B9F)
    .setDescription(textTable)
    .setImage(IMAGE_ASSETS.EMPTY)
    .setTimestamp();
