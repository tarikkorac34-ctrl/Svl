...
const TEAMS = {
  ...
  AIK:  { name: 'AIK',  emojiName: 'AlK',            fullName: 'AIK'            },
  DIF:  { name: 'DIF',  emojiName: 'DjurgardensiF',   fullName: 'Djurgårdens IF' },
  HAIF: { name: 'HAIF', emojiName: 'HammarbylF',      fullName: 'Hammarby IF'    },
  HIF:  { name: 'HIF',  emojiName: 'Helsingborgs|F',  fullName: 'Helsingborgs IF'},
  IFE:  { name: 'IFE',  emojiName: '|FElfsborg',      fullName: 'IF Elfsborg'    },
  ...
  OIS:  { name: 'ÖIS',  emojiName: 'OrgrytelS',       fullName: 'Örgryte IS'     },
};
...
async function cmdPlayer(i) {
  ...
  const teamText = hasTeam 
    ? `${teamEmoji(i.guild, p.team)} ${TEAMS[p.team]?.name || p.team}` 
    : 'N/A';
