...
// Includes Express API, Canvas Graphics & Automated Rank Request System
...
// Initialize Roblox Authentication
async function initRoblox() {
  if (ROBLOX_COOKIE) {
    try {
      ...
      console.error('❌ Roblox Authentication Failed:', err.message);
    }
  } else {
    console.warn('⚠️ ROBLOX_COOKIE is not set. Automatic Roblox ranking will be disabled.');
  }
}
...
let db, playersCol, gamesCol, pendingCol, resultsCol, refRequestsCol, rankRequestsCol;
...
async function cmdRankRequest(interaction) {
  ...
  const staffChan = findChannel(interaction.guild, 'rank-requests') || findChannel(interaction.guild, 'staff-regging');
  if (!staffChan) {
    return interaction.editReply({ content: '❌ Could not find `#rank-requests` or `#staff-regging` channel.' });
  }
  ...
  await staffChan.send({ embeds: [embed], components: [row] });
  await interaction.editReply({ content: '✅ Your rank request has been submitted to staff for review!' });
}
...
async function handleRankAccept(interaction) {
  ...
  // 1. Automatic Roblox Group Ranking
  ...
  if (ROBLOX_GROUP_ID && ROBLOX_COOKIE) {
    try {
      ...
      await noblox.setRank(ROBLOX_GROUP_ID, robloxId, rankInfo.robloxRank);
  ...
  // Remove buttons from original request message
  await interaction.message.edit({ components: [] });

  // Send output log to #rr-logs
  const logChan = findChannel(interaction.guild, 'rr-logs');
  if (logChan) {
    const logEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Roblox Username', value: req.robloxUsername, inline: true },
        { name: 'Role Requested', value: rankInfo.name, inline: true },
        { name: 'Reviewed By', value: `<@${interaction.user.id}>`, inline: false },
        { name: 'Roblox Result', value: robloxStatus, inline: true },
        { name: 'Discord Result', value: discordStatus, inline: true }
      )
      .setImage(req.proofUrl)
      .setTimestamp();
  ...
  if (logChan) {
    const logEmbed = new EmbedBuilder()
      ...
      .addFields(
        { name: 'Discord User', value: `<@${req.userId}>`, inline: true },
        { name: 'Roblox Username', value: req.robloxUsername, inline: true },
        { name: 'Role Requested', value: rankInfo ? rankInfo.name : req.roleKey, inline: true },
        { name: 'Reviewed By', value: `<@${interaction.user.id}>`, inline: false },
